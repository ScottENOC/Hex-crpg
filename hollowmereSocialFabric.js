...
// Gives Hollowmere a deliberately village-sized population, real dwellings,
// workplaces and persistent relationships. Ordinary residents are background
// people, not 80 bespoke quest-givers; the capital can therefore stay much
// denser and richer without making the starting village absurdly content-heavy.
(() => {
    'use strict';

    const TARGET_POPULATION = 80;
    const TARGET_DWELLINGS = 20; // includes existing homes + tavern lodging
    const BOOT_MS = 1200;
    const residences = [];
    const workplaces = [];
    ...
    function wallRing(floors) {
        ...
        const walls = new Map();
        floors.forEach(h => (window.getNeighbors?.(h.q,h.r) || []).forEach(n => { if (!f.has(key(n))) walls.set(key(n), n); }));
        return [...walls.values()];
    }
    ...
    function carveCottage(id, centerHex, capacity, kind = 'cottage') {
        ...
        const region = {
            ...
            minR:Math.min(...floors.map(h=>h.r)), maxR:Math.max(...floors.map(h=>h.r)), lightMult:0.3,
            doorHex:{...door}, floorHexes:floors, wallHexes:ring.filter(h=>key(h)!==key(door)), floorType:'Wood Floor',
            center:{...centerHex}, hollowmereResidenceId:id,
        };
    ...
    function carveWorkshop(id, centerHex, type, capacity) {
        ...
        const region = {
            minQ:Math.min(...floors.map(h=>h.q)), maxQ:Math.max(...floors.map(h=>h.q)),
            minR:Math.min(...floors.map(h=>h.r)), maxR:Math.max(...floors.map(h=>h.r)), lightMult:0.35,
            doorHex:{...door}, floorHexes:floors, wallHexes:ring.filter(h=>key(h)!==key(door)), floorType:'Wood Floor',
            center:{...centerHex}, hollowmereWorkplaceId:id,
        };
        window.interiorRegions.push(region);
    ...
    function buildTankardUpperFloor() {
        if (window.campaign2HollowTankardBuilding || !window.multiStoryBuildings) return !!window.campaign2HollowTankardBuilding;
        const groundCandidates = [{q:4,r:-2},{q:-4,r:-2},{q:4,r:2},{q:-4,r:2},{q:3,r:0}];
        const stair = groundCandidates.find(h => window.getTerrainAt?.(h.q,h.r)?.name === 'Wood Floor' && !window.tileObjects?.[key(h)]);
        if (!stair) return false;
        const floors = floorHexes({q:0,r:0}, 6, 4);
        const ring = wallRing(floors);
        const building = { minQ:-5, maxQ:5, minR:-6, maxR:6, floors:[] };
        building.floors[1] = { terrain:{}, tileObjects:{} };
        const upper = building.floors[1];
        ring.forEach(h => { upper.terrain[key(h)] = 'Wall'; });
        floors.forEach(h => { upper.terrain[key(h)] = 'Wood Floor'; });
        window.tileObjects[key(stair)] = { type:'stair_up', toFloor:1 };
        upper.tileObjects[key(stair)] = { type:'stair_down', toFloor:0 };
        const bedSpots = [
            {q:-4,r:-2},{q:0,r:-2},{q:4,r:-2},
            {q:-4,r:2},{q:0,r:2},{q:4,r:2},
        ].filter(h => key(h)!==key(stair));
        bedSpots.forEach((h,i) => {
            upper.tileObjects[key(h)] = { type:'bed' };
            const table = {q:h.q, r:h.r + (i<3 ? 1 : -1)};
            if (upper.terrain[key(table)] === 'Wood Floor' && !upper.tileObjects[key(table)]) upper.tileObjects[key(table)] = { type:'table' };
        });
        window.multiStoryBuildings.push(building);
        window.campaign2HollowTankardBuilding = building;
        window.campaign2HollowTankardStairHex = {...stair};
        window.campaign2HollowTankardLodgingFloor = 1;
        return true;
    }
    ...
    function expandMap() {
        ...
        buildTankardUpperFloor();
        residences.length = 0; workplaces.length = 0;
        addExistingRegistryEntries();
        const c = centre();
        const candidates = [];
        [-30,-22,30,38].forEach(q => [-26,-16,-6,16,26,36].forEach(r => candidates.push({q:c.q+q,r:c.r+r})));
        [-12,0,12].forEach(q => [-36,-46].forEach(r => candidates.push({q:c.q+q,r:c.r+r})));
        const newHomesNeeded = Math.max(0, TARGET_DWELLINGS - residences.length);
        let built = 0;
        ...
        [
            {type:'smithy',pos:{q:c.q+24,r:c.r-8},cap:5},
            {type:'bakery',pos:{q:c.q-24,r:c.r+18},cap:4},
            {type:'weaver',pos:{q:c.q+26,r:c.r+22},cap:5},
            {type:'carpenter',pos:{q:c.q-28,r:c.r-20},cap:5},
        ].forEach(w => {
            if (!canBuild(w.pos,3,2)) return;
            const shop=carveWorkshop(`hollowmere-${w.type}`,w.pos,w.type,w.cap);
            paintPathToward(shop.door,c,9);
        });
        workplaces.push(
            {id:'riverside-landing',type:'fishing',center:{q:c.q-30,r:c.r+5},door:{q:c.q-30,r:c.r+5},capacity:8},
            {id:'north-fields',type:'farm',center:{q:c.q+10,r:c.r-44},door:{q:c.q+10,r:c.r-44},capacity:18},
            {id:'south-fields',type:'farm',center:{q:c.q-12,r:c.r+48},door:{q:c.q-12,r:c.r+48},capacity:18},
            {id:'woodlot',type:'forestry',center:{q:c.q+48,r:c.r-18},door:{q:c.q+48,r:c.r-18},capacity:10},
            {id:'builders-yard',type:'labour',center:{q:c.q-34,r:c.r+30},door:{q:c.q-34,r:c.r+30},capacity:10},
        );
        window.HollowmereSettlementRegistry = {
            residences, workplaces,
            get housingCapacity(){return residences.reduce((n,r)=>n+r.capacity,0);},
            get workplaceCapacity(){return workplaces.reduce((n,w)=>n+w.capacity,0);},
        };
        mapExpanded = true;
        return true;
    }
    ...
    function takePopulationPulseOwnership() {
        ...
        populationPulseOwned = true;
    }

    function enforcePopulationTarget() {
        ...
        if (records.length > TARGET_POPULATION) {
            for (const record of records.slice(TARGET_POPULATION)) {
                ...
                p.records.delete(record.id);
            }
        }
        if (p.records.size < TARGET_POPULATION) p.ensurePopulation(TARGET_POPULATION);
    }

    function resetSocialFields(records) {
        records.forEach(r => {
            r.householdId=null; r.workplaceId=null; r.spouseId=null; r.parentIds=[]; r.childIds=[]; r.friendIds=[]; r.relationships={};
            r.isDependent = false;
        });
    }
    ...
    function assignHouseholds(records) {
        ...
        for (const residence of residences) {
            ...
            const target=Math.min(residence.capacity,remaining,3+Math.floor(hashUnit(residence.id,'household-size')*3));
            const members=records.slice(cursor,cursor+Math.max(1,target)); cursor+=members.length;
            if (!members.length) continue;
            const surname=members[0].name.split(' ').slice(-1)[0];
            const hid=`household:${residence.id}`;
            households.set(hid,{id:hid,residenceId:residence.id,homeHex:{...residence.door},surname,memberIds:members.map(m=>m.id)});
            members.forEach((m,i)=>{
                const first=m.name.split(' ')[0];
                if (residence.kind!=='lodgings' && (i<2 || hashUnit(m.seed,'same-surname')<0.86)) m.name=`${first} ${surname}`;
                m.householdId=hid; m.nodes.home={key:`home:${residence.id}`,hex:{...residence.door},floor:residence.floor||0};
            });
            const adults=members.slice(0,Math.min(2,members.length));
            if (adults.length===2) {
                const [a,b]=adults; a.spouseId=b.id; b.spouseId=a.id; a.relationships[b.id]='spouse'; b.relationships[a.id]='spouse';
                for (const child of members.slice(2)) {
                    if (hashUnit(child.seed,'dependent-child') < 0.72) {
                        child.isDependent=true; child.appearance.ageBand='child'; child.appearance.heightScale=0.68+hashUnit(child.seed,'child-height')*0.18;
                        child.parentIds=[a.id,b.id]; a.childIds.push(child.id); b.childIds.push(child.id);
                        child.relationships[a.id]='parent'; child.relationships[b.id]='parent'; a.relationships[child.id]='child'; b.relationships[child.id]='child';
                    }
                }
            }
        }
        while (cursor<records.length && households.size) {
            ...
            const res=residences.find(r=>r.id===target.residenceId); record.nodes.home={key:`home:${res.id}`,hex:{...res.door},floor:res.floor||0};
        }
    }
    ...
    function assignWorkplaces(records) {
        ...
        for (const record of records) {
            if (record.isDependent) { record.workplaceId=null; record.nodes.work={...record.nodes.home,key:`dependent:${record.householdId}`}; continue; }
            ...
            const chosen=choices[Math.min(choices.length-1,Math.floor(hashUnit(record.seed,'workplace-choice')*Math.min(3,choices.length)))];
            occupancy.set(chosen.id,(occupancy.get(chosen.id)||0)+1); record.workplaceId=chosen.id;
            record.nodes.work={key:`work:${chosen.id}`,hex:{...chosen.door}};
        }
    ...
    function assignFriends(records) {
        ...
        for (const record of records) {
            const candidates=records.filter(o=>o!==record && o.householdId!==record.householdId && distance(o.nodes.home.hex,record.nodes.home.hex)<32 && (o.isDependent===record.isDependent || !record.isDependent));
            candidates.sort((a,b)=>hashUnit(record.seed,`friend:${a.id}`)-hashUnit(record.seed,`friend:${b.id}`));
            const target=1+Math.floor(hashUnit(record.seed,'friend-count')*4);
            record.friendIds=candidates.slice(0,target).map(x=>x.id);
            record.friendIds.forEach(fid=>{ record.relationships[fid]=record.relationships[fid]||'friend'; });
            const socialTarget=byId.get(record.friendIds[0]);
            if (socialTarget) record.nodes.social={key:`visit:${socialTarget.householdId}`,hex:{...socialTarget.nodes.home.hex},floor:socialTarget.nodes.home.floor||0};
        }
    }
    ...
    function syncScheduler(records) {
        ...
        for (const record of records) {
            ...
            state.metadata={...(state.metadata||{}),householdId:record.householdId,workplaceId:record.workplaceId,dependent:!!record.isDependent};
            if (!state.travel) {
                const target=record.isDependent && pop().targetForTime(record,Number(window.worldSeconds||0))==='work' ? 'home' : pop().targetForTime(record,Number(window.worldSeconds||0));
                const node=record.nodes[target]||record.nodes.home; state.currentNode=node.key; state.currentHex={...node.hex};
            }
        }
    }
    ...
    function socialisePopulation() {
        ...
        socialisedPopulationSize=records.length; return true;
    }
