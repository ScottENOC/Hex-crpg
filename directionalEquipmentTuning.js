...
//
// Body geometry is canonical in spriteRigging.js / rig calibration. Horizontal
// armour fit is derived from torso landmarks. Vertical scale is measured from
// the armour image's visible silhouette and mapped to explicit silhouette
// EXTENTS on the body (armour shoulder top -> foot sole), never attachment
// centres such as shoulder centre -> foot centre.
...
(() => {
    ...
    function verticalBodyTarget(view) {
        ...
        // Full-body armour's top/bottom opaque pixels are silhouette edges, so
        // map them to silhouette extent anchors. Fall back to centre anchors
        // only for older rigs that do not yet define the explicit extents.
        ...
        return {
            topY,
            bottomY,
            shoulderY: topY, // compatibility aliases for existing diagnostics/tests
            footY: bottomY,
    ...
    function measureRobustAlphaBounds(image, alphaThreshold = ALPHA_THRESHOLD) {
        ...
        try {
            ...
            return {originalWidth:w,originalHeight:h,trimLeft:minX,trimTop:minY,trimWidth:maxX-minX+1,trimHeight:maxY-minY+1,alphaThreshold,minRowPixels,minColPixels,method:'row-column-occupancy'};
        } catch (_) { return null; }
    }
    ...
    function computeMeasuredArmourFit(image, view = 'front', trimOverride = null) {
        ...
        const topShift = cfg.bodyH * target.topY - trim.trimTop * scaleHexPerSourcePixel;
        ...
        return {
            view,
            trim:{...trim}, rawTrim:rawTrim?{...rawTrim}:null, target:{...target},
            ...
            source:'robust-alpha-bounds-to-armour-extents',
        };
    }
        ...
        ctx.drawImage=function(img,...args){
            if(args.length===4&&isTrackedArmourImage(img)&&isActiveHumanFemale()){
                ...
                if(fit){
                    ...
                    try { const out=previousDrawImage(img,dx,dy,dw,dh); window.HUMAN_FEMALE_EQUIPMENT_FIT.lastMeasuredArmour={...fit}; return out; }
                    finally { cfg.armour.wMult=savedWMult; cfg.armour.topShift=savedTopShift; cfg.armour.bottomDrop=savedBottomDrop; }
                }
            }
    ...
    function apply() {
        ...
        for(const view of ['front','side','back']){
            const fit=deriveArmourRigFromBody(view); if(!fit)return false; derived[view]=fit; Object.assign(directionalArmour[view],fit);
        }
        ...
        window.HUMAN_FEMALE_EQUIPMENT_FIT={
            ...
            armourScaleSource:'robust-alpha-bounds-to-armour-extents',
