...
(() => {
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', installSettingsControls, { once:true });
    } else {
        installSettingsControls();
    }
    let settingsAttempts = 0;
    const settingsTimer = setInterval(() => {
        if (installSettingsControls() || ++settingsAttempts > 40) clearInterval(settingsTimer);
    }, 250);
    ...
    function queueCanonicalAnchorOverlay(ctx, img, args) {
        ...
        queueMicrotask(() => {
            ...
            const records = anchorRecords.splice(0);
            for (const record of records) drawCanonicalAnchors(record.ctx, record.anchors);
        });
    }
    ...
    function drawCanonicalAnchors(ctx, anchors) {
        ...
        for (const p of anchors) {
            ...
            ctx.fillStyle = anchorColour(p.name);
            ctx.fill();
            ctx.strokeStyle = '#000';
            ctx.stroke();
            ctx.fillStyle = '#fff';
            ctx.strokeStyle = '#000';
            ctx.lineWidth = 3;
            ctx.strokeText(p.name,p.x+5,p.y-3);
            ctx.fillText(p.name,p.x+5,p.y-3);
        }
        ctx.restore();
    }
    ...
    function summariseDiagnostic(diag, fit) {
        const strips = diag.strips || [];
        const widths = strips.map(s => s.width).filter(Number.isFinite);
        const maxWidth = widths.length ? Math.max(...widths) : 0;
        const minWidth = widths.length ? Math.min(...widths) : 0;
        const unionLeft = strips.length ? Math.min(...strips.map(s => s.x)) : 0;
        const unionRight = strips.length ? Math.max(...strips.map(s => s.x+s.width)) : 0;
        const hsZ = (window.hexSize || 1) * (window.cameraZoom || 1);
        const fittedOuterWidthPx = fit?.outerWidth ? fit.outerWidth * hsZ : 0;
        const trim = fit?.trim || null;
        const sourceVisibleWidthPx = trim?.trimWidth || 0;
        const sourceCanvasWidthPx = trim?.originalWidth || diag.image?.naturalWidth || diag.image?.width || 0;
        const undeformedVisibleWidthPx = fittedOuterWidthPx && sourceCanvasWidthPx
            ? fittedOuterWidthPx * sourceVisibleWidthPx / sourceCanvasWidthPx : 0;
        const compression = fittedOuterWidthPx ? maxWidth / fittedOuterWidthPx : 0;
        const result = {
            view: fit?.view || diag.view || 'unknown',
            sourceVisibleWidthPx,
            sourceCanvasWidthPx,
            sourceVisibleHeightPx: trim?.trimHeight || 0,
            fittedOuterWidthPx,
            fittedOuterWidthHex: fit?.outerWidth || 0,
            fittedOuterHeightHex: fit?.outerHeight || 0,
            undeformedVisibleWidthPx,
            stripCount: strips.length,
            stripMinWidthPx:minWidth,
            stripMaxWidthPx:maxWidth,
            stripUnionWidthPx:Math.max(0,unionRight-unionLeft),
            stripCompressionRatio:compression,
            stripWidthsPx:widths,
            timestamp:Date.now(),
        };
        window.__lastArmourWidthDebug = result;
        updateDiagnosticPanel();
        return result;
    }

    function ensureDiagnosticPanel() {
        let panel = document.getElementById('armour-width-debug-panel');
        if (panel) return panel;
        panel = document.createElement('div');
        panel.id = 'armour-width-debug-panel';
        Object.assign(panel.style, {
            position:'fixed', right:'6px', top:'calc(env(safe-area-inset-top, 0px) + 6px)', zIndex:'100000',
            maxWidth:'300px', padding:'7px 9px', border:'1px solid #00e5ff', borderRadius:'5px',
            background:'rgba(0,0,0,.82)', color:'#d7ffff', font:'11px/1.25 monospace', whiteSpace:'pre-wrap',
    ...
    function updateDiagnosticPanel() {
        ...
        if (!window.showArmourWidthDebug) { panel.style.display='none'; return; }
        panel.style.display='block';
        const d = window.__lastArmourWidthDebug;
        if (!d) { panel.textContent='Armour width diagnostic\nWaiting for armour draw…'; return; }
        panel.textContent = [
            `Armour width — ${d.view}`,
            `source visible: ${d.sourceVisibleWidthPx}/${d.sourceCanvasWidthPx}px`,
            `fitted box: ${d.fittedOuterWidthPx.toFixed(1)}px (${d.fittedOuterWidthHex.toFixed(3)} hex)`,
            `visible before strips: ${d.undeformedVisibleWidthPx.toFixed(1)}px`,
            `strips: ${d.stripCount}  min/max ${d.stripMinWidthPx.toFixed(1)}/${d.stripMaxWidthPx.toFixed(1)}px`,
            `strip union: ${d.stripUnionWidthPx.toFixed(1)}px`,
            `box retained: ${(d.stripCompressionRatio*100).toFixed(1)}%`,
        ].join('\n');
    }

    function installOuterDiagnosticWrapper() {
        const ctx = window.mapCtx;
        if (!ctx || ctx.drawImage?.__rigDebugOuter) return !!ctx;
        ...
        const wrapper = function(img, ...args) {
            ...
            const beforeFit = window.HUMAN_FEMALE_EQUIPMENT_FIT?.lastMeasuredArmour;
            const diag = { image:img, strips:[], view:null };
            const saved = activeArmourDiagnostic;
            activeArmourDiagnostic = diag;
            try {
                return previous(img,...args);
            } finally {
                activeArmourDiagnostic = saved;
                const fit = window.HUMAN_FEMALE_EQUIPMENT_FIT?.lastMeasuredArmour;
                if (fit && fit !== beforeFit) summariseDiagnostic(diag, fit);
            }
        };
        wrapper.__rigDebugOuter = true;
        wrapper.__rigDebugPrevious = previous;
    ...
    const timer=setInterval(() => {
        attempts++;
        if (installOuterDiagnosticWrapper() || attempts > 100) clearInterval(timer);
    },100);
    if (document.readyState === 'complete') installOuterDiagnosticWrapper();
    else window.addEventListener('load', () => setTimeout(installOuterDiagnosticWrapper,0), {once:true});
